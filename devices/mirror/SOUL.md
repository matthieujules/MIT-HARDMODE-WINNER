...
### `show_preset` — Pre-made images (fast, no generation needed)
You have a library of high-quality pre-rendered images. When the instruction matches one of these, use `show_preset` instead of `display` or `edit_photo` — it's instant (no API call, no generation wait). The available presets are loaded dynamically from your images folder.
